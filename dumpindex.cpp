...
void document_text(indri::collection::Repository& r, const char* an, int argc, char** argv)
{
 indri::collection::CompressedCollection* collection = r.collection();
std::string attributeName = an;
for (int i = 4; i <argc; i++)
{
  std::string attributeValue = argv[i];
  ...
  for( size_t i=0; i<documentIDs.size(); i++ ) {
   ...
   indri::api::ParsedDocument* document = collection->retrieve( documentIDs[i] );
   std::cout << document->text << std::endl;
   delete document;
  }
...
int main( int argc, char** argv ) {
  try {
    ...
    if( command == "c" || command == "compact" ) {
      ...
      } else if(command == "doct" || command == "doctext") {
        REQUIRE_ARGS(5);
        document_text(r,argv[3],argc,argv);
